...
export default function SaveMyExamsPage() {
  ...
  const loadResources = async () => {
    try {
      setLoading(true);
      const data = await fetcher('https://node.snapaper.com/api/savemyexams');
      setResources(data.savemyexams || []);
      setResourceCount(data.count || 0);
      setError(null);
    } catch (err) {
      setError(err.message);
      notification.error({
        message: 'Error',
        description: 'Failed to load Save My Exams resources. Please try again later.',
      });
    } finally {
      setLoading(false);
    }
      ...
      <main className="ant-container">
        <section className="next-cate-header next-topic-header">
          ...
          <div className="next-paper-header-info">
            <div>
              <Button>{loading ? 'Loading...' : `${resourceCount} Resources`}</Button>
