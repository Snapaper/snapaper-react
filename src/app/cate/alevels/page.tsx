...
const ALevelsPage: React.FC = () => {
  ...
  const fetchSubjects = async () => {
    try {
      setLoading(true);
      const response = await axios.get(config.apiUrl.cates.alevel);
  ...
  const fetchYears = async (subject: string) => {
    try {
      ...
      const response = await axios.get(config.apiUrl.years.alevel + subject);
      ...
      <div className="flex-1">
        <main className="container-main">
          ...
          <section className="section-padding">
            <div className="bg-gradient-to-r from-cyan-50 to-teal-50 rounded-2xl p-8 md:p-12 shadow-lg">
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {subjects.map((subject, index) => (
                  <Card
                    key={index}
                    hoverable
                    className="subject-card subject-card-alevel cursor-pointer transform transition-all duration-300 hover:scale-105"
                    onClick={() => handleSubjectClick(subject)}
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        <h3 className="text-lg font-semibold text-gray-900 mb-2">
                          <BookOutlined className="mr-2 text-secondary" />
        ...
        {yearsLoading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 py-4">
            {years.map((year, index) => (
              <Link
                key={index}
                href={`/paper/alevels/ppca/${year.name}/${selectedSubject}`}
